                ...
                <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                    ...
                    <div class="flex gap-1 p-1 bg-slate-100 dark:bg-slate-800/50 rounded-xl overflow-x-auto max-w-full">
                        <a href="{{ route('admin.analytics', array_merge(request()->query(), ['role' => 'all'])) }}"
                            class="px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all whitespace-nowrap {{ !request('role') || request('role') === 'all' ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300' }}">
                            All
                        </a>
                        @foreach(['user', 'agent', 'dealer', 'super_agent', 'admin'] as $role)
                            <a href="{{ route('admin.analytics', array_merge(request()->query(), ['role' => $role])) }}"
                                class="px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all whitespace-nowrap {{ request('role') === $role ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300' }}">
                                {{ str_replace('_', ' ', $role) }}
                            </a>
                        @endforeach
                    </div>

                    <form action="{{ route('admin.analytics') }}" method="GET" class="relative">
                        @if(request('role'))
                            ...
                            <tr class="group hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-all">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="text-sm font-bold text-slate-900 dark:text-white">{{ $reseller->name }}
                                            </p>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <span
                                        class="text-sm font-bold text-slate-700 dark:text-slate-300">{{ number_format($reseller->total_orders) }}</span>
