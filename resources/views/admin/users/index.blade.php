...
@section('content')
    <div class="space-y-6 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700" x-data="{
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-4 w-full md:w-auto">
                {{-- Search --}}
                <form action="{{ route('admin.users') }}" method="GET" class="flex gap-4 items-center flex-1">
        ...
        <div class="flex gap-2 p-1 bg-slate-100 dark:bg-slate-800/50 rounded-xl w-fit overflow-x-auto">
            <a href="{{ route('admin.users') }}"
                class="px-4 py-2 rounded-lg text-xs font-bold transition-all {{ !request('role') ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300' }}">
                All Users
            </a>
            @foreach(['admin', 'user', 'agent', 'dealer', 'super_agent'] as $role)
                <a href="{{ route('admin.users', ['role' => $role]) }}"
                    class="px-4 py-2 rounded-lg text-xs font-bold transition-all capitalize {{ request('role') === $role ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300' }}">
                    {{ str_replace('_', ' ', $role) }}
                </a>
            @endforeach
        </div>
                    ...
                    <tbody class="divide-y divide-slate-50 dark:divide-slate-800">
                        @forelse($users as $u)
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-slate-500 dark:text-slate-500 mt-1.5">{{ $u->email }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    ...
                                    @endphp
                                    <span
                                        class="px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-tight {{ $rc }}">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <span
                                        class="text-xs font-medium text-slate-500 dark:text-slate-500">{{ $u->created_at->format('M d, Y') }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form action="{{ route('admin.users.toggle-status', $u->id) }}" method="POST">
                                            @csrf
                                            <button type="submit" title="{{ $u->is_active ? 'Suspend User' : 'Activate User' }}"
                                                class="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-800 hover:bg-{{ $u->is_active ? 'orange' : 'green' }}-50 dark:hover:bg-{{ $u->is_active ? 'orange' : 'green' }}-900/30 text-slate-400 hover:text-{{ $u->is_active ? 'orange' : 'green' }}-600 transition-all flex items-center justify-center">
                                                @if($u->is_active)
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        {{-- View Orders --}}
                                        <a href="{{ route('admin.orders', ['user_id' => $u->id]) }}" title="View User Orders"
                                        ...
                                        {{-- Credit/Debit Wallet --}}
                                        <button @click="openWallet({{ $u->toJson() }})" title="Adjust Wallet Balance"
        ...
        <div x-show="modalOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4" x-cloak>
            ...
            <div
                ...
                <form :action="editMode ? '{{ url('admin/users') }}/' + user.id : '{{ route('admin.users.store') }}'"
    ...
    @include('admin.users.wallet-modal')
@endsection