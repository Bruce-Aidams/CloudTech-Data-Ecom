    ...
    <div class="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div class="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div
                ...
                <div class="flex items-baseline gap-1">
                    ...
                    <p class="text-3xl font-black text-white tracking-tighter">
                        {{ number_format($stats['total_credits'], 2) }}</p>
                </div>
            </div>
            ...
            <div
                ...
                <div class="flex items-baseline gap-1">
                    <span class="text-sm font-black text-white/50 uppercase">GHC</span>
                    <p class="text-3xl font-black text-white tracking-tighter">{{ number_format($stats['total_spent'], 2) }}
                    </p>
                </div>
            </div>
            ...
            <div
                ...
                <div class="flex items-baseline gap-1">
                    <span class="text-sm font-black text-white/50 uppercase">GHC</span>
                    <p class="text-3xl font-black text-white tracking-tighter">{{ number_format($stats['this_month'], 2) }}
                    </p>
                </div>
            </div>
        </div>
                ...
                <form action="{{ route('billing.index') }}" method="GET"
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Ref or task..."
                            ...
                            <div class="p-6 space-y-4">
                                <div class="flex items-center justify-between">
                                    <div class="flex flex-col gap-1">
                                        ...
                                        <p class="font-mono text-[9px] font-black text-primary tracking-widest">
                                            #{{ strtoupper(substr($transaction->reference, 0, 10)) }}</p>
                                    </div>
                                    <span
                                        class="text-lg font-black tracking-tighter {{ $transaction->type === 'credit' ? 'text-emerald-600' : 'text-rose-600' }}">
                                        {{ $transaction->type === 'credit' ? '+' : '-' }}GHC {{ number_format($transaction->amount, 2) }}
                                    </span>
                                </div>
                                <div class="bg-slate-50 dark:bg-slate-800/40 rounded-3xl p-5 space-y-4">
                                    ...
                                    <div
                                        ...
                                        <div class="flex items-center gap-2">
                                            <div
                                                class="h-2 w-2 rounded-full {{ $transaction->status === 'success' || $transaction->status === 'completed' ? 'bg-emerald-500 animate-pulse' : ($transaction->status === 'pending' ? 'bg-amber-500' : 'bg-rose-500') }}">
                                            </div>
                                            <p class="text-[9px] font-black uppercase text-slate-500 tracking-widest">
                                                {{ $transaction->status === 'success' ? 'Successful' : ($transaction->status === 'pending' ? 'Pending' : ucfirst($transaction->status)) }}
                                            </p>
                                        </div>
                                ...
                                @foreach($transactions as $transaction)
                                    <tr class="group hover:bg-white dark:hover:bg-slate-800/50 transition-all duration-300">
                                        ...
                                        <td class="px-10 py-6">
                                            <div class="flex items-center gap-2">
                                                <div
                                                    class="w-2 h-2 rounded-full {{ $transaction->type === 'credit' ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500' }}">
                                                </div>
                                                <span
                                                    class="text-base font-black tracking-tighter {{ $transaction->type === 'credit' ? 'text-emerald-600' : 'text-rose-600' }}">
                                                    {{ $transaction->type === 'credit' ? '+' : '-' }}GHC {{ number_format($transaction->amount, 2) }}
                                                </span>
                                            </div>
                                        </td>
                                        <td class="px-10 py-6">
                                            ...
                                            @else
                                                <span
                                                    class="inline-flex items-center px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400">{{ $transaction->status }}</span>
                                            @endif
                                        </td>
