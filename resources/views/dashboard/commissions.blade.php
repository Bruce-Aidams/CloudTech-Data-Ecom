        ...
        <div x-show="withdrawOpen" x-cloak class="relative z-50">
            ...
            <div x-show="withdrawOpen" x-transition:enter="transition ease-out duration-300"
                ...
                <div @click.away="withdrawOpen = false"
                    ...
                    <form action="{{ route('payouts.store') }}" method="POST" class="space-y-6">
                        @csrf
                        <div class="space-y-2">
        ...
        <div class="grid gap-8 md:grid-cols-3">
            ...
            <div
                ...
                <div class="relative z-10 text-white">
                    ...
                    <p class="text-4xl font-black tracking-tighter">GHS {{ number_format($balance, 2) }}</p>
                </div>
            </div>
            ...
            <div
                ...
                <p class="text-4xl font-black text-white tracking-tighter">{{ $stats['total_referrals'] }} <span
                        ...
                        @forelse($commissions as $commission)
                            <div
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <span
                                        class="text-[8px] font-black text-slate-400 uppercase tracking-widest">{{ $commission->created_at->format('M d') }}</span>
                                </div>
                                <div class="space-y-1">
                                    <p
                    ...
                    <div class="md:hidden grid grid-cols-2 gap-3 px-6 pb-8">
                        @forelse($payouts as $payout)
                            <div
                                ...
                                <div class="flex items-center justify-between">
                                    <span
                                        class="font-mono text-[8px] font-black text-primary/60 tracking-tighter">#{{ strtoupper(substr($payout->reference, 0, 6)) }}</span>
                                    <span
                                        class="text-[8px] font-black text-slate-400 uppercase tracking-widest">{{ $payout->created_at->format('M d') }}</span>
                                </div>
                                <div class="space-y-2">
                                ...
                                @forelse($payouts as $payout)
                                    <tr class="group hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-all">
                                        <td class="px-8 py-6 font-mono text-[9px] font-black text-primary tracking-widest">
                                            #{{ strtoupper(substr($payout->reference, 0, 8)) }}
                                        </td>
                                        <td class="px-8 py-6 font-black text-foreground">
                                            GHS {{ number_format($payout->amount, 2) }}
                                        </td>
                                        <td class="px-8 py-6">
                                            @if($payout->status === 'completed')
                                                <span
                                            ...
                                            @else
                                                <span
                                                    class="inline-flex items-center px-3 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400">{{ $payout->status }}</span>
                                            @endif
                                        </td>
                                        <td
