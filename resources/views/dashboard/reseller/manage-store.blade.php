    ...
    <div class="space-y-8 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div class="flex items-center gap-4">
                {{-- Store Status Toggle --}}
                <form action="{{ route('reseller.store.toggle') }}" method="POST"
                    ...
                    <button type="submit"
                        class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors {{ $user->store_active ? 'bg-emerald-500' : 'bg-slate-200 dark:bg-slate-700' }}">
                        <span
                            class="inline-block h-4 w-4 transform rounded-full bg-white transition {{ $user->store_active ? 'translate-x-6' : 'translate-x-1' }}"></span>
                    </button>
                </form>
            ...
            <div class="relative z-10 p-8 md:p-10 space-y-6">
                ...
                <div class="flex flex-col lg:flex-row items-center gap-4">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
                        ...
                        <form action="{{ route('reseller.store.regenerate') }}" method="POST"
                ...
                <div
                    ...
                    <div
                        ...
                        <div class="absolute top-4 left-4 z-10">
                            ...
                            @endphp
                            <span
                                class="px-3 py-1 rounded-xl text-[10px] font-black tracking-widest uppercase shadow-lg backdrop-blur-md {{ $nc }}">
                                {{ $bundle->network }}
                            </span>
                        </div>
                        ...
                        <div
                            ...
                            <div class="flex items-baseline gap-1">
                                ...
                                <span
                                    class="text-sm font-black text-slate-900 dark:text-white tabular-nums">₵{{ number_format($bundle->cost_to_reseller, 2) }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <form action="{{ route('reseller.store.update-price') }}" method="POST" class="space-y-4">
                            @csrf
                                    ...
                                    @if($bundle->profit_per_unit > 0)
                                        <div class="flex items-center gap-1 text-emerald-600 dark:text-emerald-400">
                                            ...
                                            <span class="text-[10px] font-black uppercase tracking-wide">Profit:
                                                ₵{{ number_format($bundle->profit_per_unit, 2) }}</span>
                                        </div>
                                    @endif
                                </div>
