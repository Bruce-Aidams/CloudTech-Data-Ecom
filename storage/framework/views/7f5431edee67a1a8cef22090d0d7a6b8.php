...
<?php $__env->startSection('content'); ?>
    <div class="space-y-6 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700" x-data="{ 
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <form action="<?php echo e(route('admin.deposits')); ?>" method="GET" class="relative group w-full lg:max-w-md">
                ...
                <input type="text" name="search" placeholder="Search customer, reference or amount..." value="<?php echo e(request('search')); ?>"
            ...
            <div class="flex gap-2 p-1 bg-slate-100 dark:bg-slate-800/50 rounded-xl w-fit overflow-x-auto">
                <a href="<?php echo e(route('admin.deposits')); ?>"
                ...
                <?php $__currentLoopData = ['pending', 'approved', 'rejected']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $status): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <a href="<?php echo e(route('admin.deposits', ['status' => $status])); ?>"
                        ...
                        <?php echo e($status === 'pending' ? 'Validating' : $status); ?>
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
            ...
            <div class="md:hidden grid grid-cols-2 gap-3 p-4">
                <?php $__empty_1 = true; $__currentLoopData = $deposits; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $d): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 shadow-sm space-y-3 relative overflow-hidden group">
                        <div class="flex items-center justify-between">
                            <div class="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 font-black text-[10px]">
                                <?php echo e(strtoupper(substr($d->user->name, 0, 1))); ?>

                            </div>
                            <span class="text-[8px] font-black text-slate-400 uppercase tracking-widest"><?php echo e($d->created_at->format('M d')); ?></span>
                        </div>

                        <div class="space-y-1">
                            <p class="text-[10px] font-black text-slate-900 dark:text-white uppercase truncate leading-none"><?php echo e($d->user->name); ?></p>
                            <p class="text-xs font-black tracking-tighter text-primary">GHS <?php echo e(number_format($d->amount, 2)); ?></p>
                        </div>
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            ?>
                            <div class="w-full text-center py-1.5 rounded-lg <?php echo e($sc); ?> text-[8px] font-black uppercase tracking-widest">
                                <?php echo e($d->status === 'pending' ? 'Validating' : $d->status); ?>

                            </div>

                            <?php if($d->status === 'pending'): ?>
                                <button @click="openProcess(<?php echo e($d->toJson()); ?>)"
                                    class="w-full h-8 bg-slate-900 dark:bg-primary text-white rounded-lg font-black text-[8px] uppercase shadow-sm active:scale-95 transition-all">
                                    Process
                                </button>
                            <?php endif; ?>
                        </div>
                    ...
                    <tbody class="divide-y divide-slate-50 dark:divide-slate-800">
                        <?php $__empty_1 = true; $__currentLoopData = $deposits; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $d): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                                <td class="px-6 py-4">
                                    <span class="font-bold text-sm text-slate-900 dark:text-white block"><?php echo e($d->created_at->format('d M, Y')); ?></span>
                                    <span class="text-[10px] text-slate-500 dark:text-slate-500 mt-1 uppercase"><?php echo e($d->created_at->format('h:i A')); ?></span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="font-bold text-sm text-slate-900 dark:text-white leading-none"><?php echo e($d->user->name); ?></p>
                                            <p class="text-[10px] text-slate-500 dark:text-slate-500 mt-1"><?php echo e($d->user->email); ?></p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <p class="text-sm font-bold text-slate-900 dark:text-white">GHC <?php echo e(number_format($d->amount, 2)); ?></p>
                                </td>
                                <td class="px-6 py-4">
                                    <?php if($d->proof_image === 'paystack'): ?>
                                    ...
                                    <?php elseif($d->proof_image): ?>
                                        <a href="<?php echo e(asset('storage/' . $d->proof_image)); ?>" target="_blank"
                                ...
                                <td class="px-6 py-4 text-center">
                                    <?php
                                        ...
                                        $sc = $statuses[$d->status] ?? 'bg-slate-50 dark:bg-slate-800 text-slate-600';
                                    ?>
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-tight <?php echo e($sc); ?>">
                                        <?php echo e($d->status === 'pending' ? 'Validating' : $d->status); ?>
            ...
            <?php if($deposits->isNotEmpty()): ?>
                <div class="px-6 py-4 border-t border-slate-50 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/20 flex items-center justify-between">
                    <div class="text-xs font-bold text-slate-500 dark:text-slate-500 uppercase tracking-widest italic">Ledger Entries: <?php echo e($deposits->total()); ?></div>
                    <div><?php echo e($deposits->links()); ?></div>
                </div>
            <?php endif; ?>
        </div>
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div x-show="deposit.proof_image && deposit.proof_image !== 'paystack'" class="space-y-1.5">
                        ...
                        <div class="bg-slate-100 dark:bg-slate-800 rounded-2xl overflow-hidden aspect-video relative group border border-slate-200 dark:border-slate-700">
                            <img :src="'<?php echo e(asset('storage')); ?>/' + deposit.proof_image" class="w-full h-full object-cover">
                            <a :href="'<?php echo e(asset('storage')); ?>/' + deposit.proof_image" target="_blank"
    ...
    <?php $__env->startPush('scripts'); ?>
        <script>
            async function handleDeposit(action) {
                ...
                try {
                    const response = await fetch(`<?php echo e(url('admin/deposits')); ?>/${depositId}/${action}`, {
                        method: 'POST',
                        headers: {
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Bruce\Desktop\Projects\Megaai2\Megaai\cloudtech\resources\views/admin/deposits/index.blade.php ENDPATH**/ ?>